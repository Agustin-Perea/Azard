...
class_name PerfectCrimeItemEffect
...
func on_item_use(roulette_controller: RouletteController) -> void:
	if roulette_controller == null:
		return
	if not _has_winning_precision_chip(roulette_controller):
		return
	...
	BookEventBus.turn_log_entry.emit("PerfectCrime: +" + str(BASE_BONUS) + " base | ignora shield", Color(0.48, 1.0, 0.76, 1.0))
...
func _has_winning_precision_chip(roulette_controller: RouletteController) -> bool:
	...
	for field_id in active_bets:
		...
		if field == null or not _is_precision_field(field):
			continue
		if roulette_controller._active_ball_matches_bet_field(roulette_controller.winner_betfield_model, field):
			return true
	return false

func _is_precision_field(field: BetFieldModel) -> bool:
	if field == null or field.ConditionStrategy == null:
		return false
	return field.ConditionStrategy is StraightUpCondition \
		or field.ConditionStrategy is FirstRowCondition \
		or field.ConditionStrategy is SecondRowCondition \
		or field.ConditionStrategy is ThirdRowCondition \
		or field.ConditionStrategy is FirstColumnCondition \
		or field.ConditionStrategy is SecondColumnCondition \
		or field.ConditionStrategy is ThirdColumnCondition
