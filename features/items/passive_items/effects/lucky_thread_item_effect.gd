...
class_name LuckyThreadItemEffect

const REQUIRED_DISTINCT_FIELDS := 2
...
func on_item_use(roulette_controller: RouletteController) -> void:
	...
	BookEventBus.turn_log_entry.emit("LuckyThread: +" + _format_number(bonus) + " mult", Color(0.75, 0.52, 1.0, 1.0))
...
func _on_bet_post_resolved(roulette_controller: RouletteController) -> void:
	...
	if distinct_fields < REQUIRED_DISTINCT_FIELDS:
		return
	pending_mult_bonus = MULT_BONUS * runtime_quantity
