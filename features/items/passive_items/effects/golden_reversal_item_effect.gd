...
class_name GoldenReversalItemEffect

const GOLD_BASE_FACTOR := 3
...
func on_item_use(roulette_controller: RouletteController) -> void:
	if roulette_controller == null:
		return
	if _has_winning_chip(roulette_controller):
		return
	pending_gold = maxi(1, int(round(roulette_controller.base)) * GOLD_BASE_FACTOR)
	pending_activation = true
	animate.emit()
	BookEventBus.turn_log_entry.emit("GoldenReversal: +" + str(pending_gold) + " Gold si confirmas", Color(1.0, 0.86, 0.18, 1.0))
...
func _on_bet_post_resolved(roulette_controller: RouletteController) -> void:
	if used_this_combat or pending_activation:
		return
...
func _on_attack_committed(_roulette_controller: RouletteController) -> void:
	if not pending_activation or pending_gold <= 0:
		_clear_pending_resolution()
		return
	if GameState.economy_component != null:
		GameState.economy_component.add_run_gold(pending_gold)
	used_this_combat = true
	BookEventBus.turn_log_entry.emit("GoldenReversal: +" + str(pending_gold) + " Gold", Color(1.0, 0.86, 0.18, 1.0))
	_clear_pending_resolution()
