...
func _ready() -> void:
	...
	UiEventBus.clear_passive_items_panels.emit()
	continue_button.disabled = not GameState.GamePersistence.has_save()
	continue_button.modulate.a = 1.0 if GameState.GamePersistence.has_save() else 0.45
...
func _start_new_run() -> void:
	...
	GameState.new_run()
	_go_to_scene(Constants.MAP_SCENE_PATH)

func _on_continue_button_pressed() -> void:

	if not GameState.GamePersistence.has_save():
		return
	_play_click()
